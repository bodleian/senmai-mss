...
<xsl:stylesheet xmlns="http://www.tei-c.org/ns/1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="path">
    <xsl:value-of
      select="concat('file:///home/morri/workspace/', $catdir, '-mss/collections_new/?select=', $files,';on-error=warning;recurse=',$recurse)"
    />
  </xsl:variable>
  ...
  <xsl:template name="main">
    ...
    <xsl:for-each select="$doc">
      ...
      <xsl:variable name="outputFilename"
          select="concat('file:///home/morri/workspace/', $catdir, '-mss/collections/', $msID, '.xml')"/>
      <!-- create output file -->
        ...
        <xsl:otherwise>
          <revisionDesc>
            <change when="{substring(string(current-date()), 0, 11)}">
              <persName>Andrew Morrison</persName> Re-ran James Cummings' up-convertion script <ref
                target="{concat('https://github.com/bodleian/', $catdir, '-mss/tree/master/processing/batch_conversion/convertShan2Bodley.xsl')}"
                  >convertShanBodley.xsl</ref>
            </change>
          </revisionDesc>
        </xsl:otherwise>
      </xsl:choose>
